...
verify_design_evidence() {
  ...
  local manifest="$screenshot_dir/manifest.sha256"
  local screenshot_count=0 file header byte_count digest width_hex height_hex
  local width height expected
  local expected_screens=(
    01-home.png
    02-tournaments.png
    03-clubs.png
    04-tournament-detail.png
    05-context-chat.png
    06-profile-records.png
    07-club-applicant.png
    08-club-management.png
    09-login-error.png
    10-full-chat.png
    11-more.png
    12-notifications.png
    13-favorites.png
    14-profile-settings.png
    15-rules.png
    16-blocked-users.png
    17-tournament-submit.png
    18-signup-age.png
  )

  [[ -d "$screenshot_dir" ]] || {
    echo "디자인 스크린샷 디렉터리가 없습니다." >&2
    return 1
  }
  screenshot_count="$(
    find "$screenshot_dir" -maxdepth 1 -type f -name '*.png' | awk 'END {print NR}'
  )"
  if [[ "$screenshot_count" != "${#expected_screens[@]}" ]]; then
    echo "디자인 스크린샷은 ${#expected_screens[@]}장이 필요하지만 ${screenshot_count}장입니다." >&2
    return 1
  fi

  for expected in "${expected_screens[@]}"; do
    [[ -f "$screenshot_dir/$expected" ]] || {
      echo "필수 디자인 스크린샷이 없습니다: $expected" >&2
      return 1
    }
  done
  ...
  while IFS= read -r file; do
    header="$(xxd -p -l 8 "$file")" || return 1
    byte_count="$(wc -c < "$file" | tr -d '[:space:]')" || return 1
    if [[ "$header" != "89504e470d0a1a0a" ]] || (( byte_count < 1024 )); then
      echo "유효하지 않은 PNG 증거입니다: $file" >&2
      return 1
    fi
    width_hex="$(xxd -p -s 16 -l 4 "$file")" || return 1
    height_hex="$(xxd -p -s 20 -l 4 "$file")" || return 1
    width=$((16#$width_hex))
    height=$((16#$height_hex))
    if (( width != 390 || height != 844 )); then
      echo "디자인 스크린샷 해상도는 390x844여야 합니다: $(basename "$file") ${width}x${height}" >&2
      return 1
    fi
    digest="$(shasum -a 256 "$file" | awk '{print $1}')" || return 1
    printf '%s  %s\n' "$digest" "$(basename "$file")" >> "$manifest" || return 1
  done < <(find "$screenshot_dir" -maxdepth 1 -type f -name '*.png' | sort)

  echo "디자인 스크린샷 ${screenshot_count}장, 390x844 해상도와 SHA-256 manifest 확인 완료"
}
...
run_artifact_credential_scan() {
  ...
  [[ -d "$artifact_root" ]] || {
    echo "artifact 디렉터리가 없습니다: $artifact_root" >&2
    printf 'failed\n' > "$status_file" || true
    return 1
  }
  ...
  matches="$(rg -l --hidden -e "$pattern" "$artifact_root" 2>/dev/null)"
  scan_exit=$?
  if (( scan_exit > 1 )); then
  ...
  if [[ -n "$matches" ]]; then
    ...
    remaining="$(rg -l --hidden -e "$pattern" "$artifact_root" 2>/dev/null)"
    scan_exit=$?
    if (( scan_exit > 1 )); then
...
write_manifest() {
  ...
  local working_tree_status working_tree_dirty_json working_tree_fingerprint
  commit_sha="$(git rev-parse HEAD)" || return 1
  branch="$(git branch --show-current)" || return 1
  working_tree_status="$(git status --porcelain=v1 --untracked-files=all)" || return 1
  if [[ -n "$working_tree_status" ]]; then
    working_tree_dirty_json=true
    working_tree_fingerprint="$(printf '%s' "$working_tree_status" | shasum -a 256 | awk '{print $1}')" || return 1
  else
    working_tree_dirty_json=false
  ...
  printf '{\n  "run_id": "%s",\n  "commit_sha": "%s",\n  "branch": "%s",\n  "working_tree_dirty": %s,\n  "working_tree_fingerprint": "%s",\n  "mode": "observe-only",\n  "persona_count": 8,\n  "reset_requested": %s,\n  "reset_completed": %s,\n  "data_provenance": "%s",\n  "contains_real_personal_data": %s\n}\n' \
    "$(json_escape "$RUN_ID")" \
    "$(json_escape "$commit_sha")" \
    "$(json_escape "$branch")" \
    "$working_tree_dirty_json" \
    "$(json_escape "$working_tree_fingerprint")" \
    "$requested_json" \
    "$completed_json" \
...
write_summary() {
  local overall step status artifact_scan_status screenshot_count
  artifact_scan_status="$(sed -n '1p' "$ARTIFACT_SCAN_STATUS_FILE" 2>/dev/null || true)"
  screenshot_count="$(
    find "$ARTIFACT_DIR/screenshots" -maxdepth 1 -type f -name '*.png' \
  ...
  if (( FAILED_STEPS == 0 )); then overall="PASS"; else overall="FAIL"; fi
  {
    ...
    echo "- 디자인 스크린샷: ${screenshot_count}장"
    echo
    echo "## 단계별 결과"
    echo
...
run_step "storage-smoke" "storage_privacy" 180 bash scripts/qa/smoke_storage.sh || true
run_step "account-deletion-smoke" "privacy_deletion" 240 bash scripts/qa/smoke_account_deletion.sh || true
...
run_step "common-harness" "code_quality" 1800 run_common_harness || true
run_step "flutter-e2e-web" "application_e2e" 1200 \
  env ALLROUND_E2E_SCREENSHOT_DIR="$ARTIFACT_DIR/screenshots" \
  bash scripts/qa/run_flutter_e2e.sh --device chrome || true
run_step "design-evidence" "visual_regression" 60 \
  verify_design_evidence "$ARTIFACT_DIR/screenshots" || true
