...
printf '\n'; echo "[2/9] grades parity (DB 정본 대조)"
...
printf '\n'; echo "[4/9] division parity (DB 정본 대조)"
if psql "$DB_URL" -c 'select 1' >/dev/null 2>&1; then
...
printf '\n'; echo "[9/9] Deno Edge Function checks"
