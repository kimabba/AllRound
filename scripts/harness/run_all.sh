...
printf '\n'; echo "[2/11] grades parity (DB 정본 대조)"
...
printf '\n'; echo "[4/11] division parity (DB 정본 대조)"
if psql "$DB_URL" -c 'select 1' >/dev/null 2>&1; then
...
# 지역 정본↔폴백 대조. 지역 3층 정규식 검사(check_enums.py)를 걷어낸 자리를 협회와
# 같은 스냅샷 다리로 메운다(P7). regions 엔 sort_order 가 없어 code 순으로 값만 대조한다.
printf '\n'; echo "[5/11] region parity (DB 정본 대조)"
if psql "$DB_URL" -c 'select 1' >/dev/null 2>&1; then
  DATABASE_URL="$DB_URL" python3 scripts/harness/check_region_parity.py
else
  echo "⚠️  로컬 DB 미접속 — region parity 건너뜀(supabase start 후 재실행하거나 CI 가 강제)" >&2
fi
# pgTAP 은 CI 에만 있어서 로컬 harness 가 초록불이어도 DB 제약·트리거 회귀를 놓쳤다
# (JY-135: 새 트리거가 QA 페르소나 시드를 막는 걸 CI 에서야 발견). 같은 조건부로 태운다.
printf '\n'; echo "[6/11] database pgTAP"
if psql "$DB_URL" -c 'select 1' >/dev/null 2>&1; then
...
printf '\n'; echo "[11/11] Deno Edge Function checks"
