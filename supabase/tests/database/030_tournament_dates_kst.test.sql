...
-- 같으므로, 둘 다 current_date 로 되돌아가면 낮 시간대엔 이 동작 테스트가 통과해
-- 버린다 — 그래서 prosrc 에 current_date 가 없다는 정적 검사를 함께 둔다.
-- 회귀를 실제로 잡는 건 그 두 항목이다.
...
select is(
  (select bool_or(p.prosrc like '%current_date%')
     from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public' and p.proname = 'tournament_search_by_slots'),
  false, 'tournament_search_by_slots 는 current_date(=UTC) 를 쓰지 않는다');

select is(
  (select bool_or(p.prosrc like '%current_date%')
     from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public' and p.proname = 'tournaments_for_user'),
  false, 'tournaments_for_user 는 current_date(=UTC) 를 쓰지 않는다');
