...
SELECT throws_ok(
  $$INSERT INTO public.chat_messages
      (user_id, conversation_id, role, content)
    VALUES
      ('00000000-0000-4000-8000-000000000005', gen_random_uuid(), 'user', '직접 삽입 시도')$$,
  '42501',
  'new row violates row-level security policy for table "chat_messages"',
  '본인 대화도 검증된 채팅 Edge Function 밖에서 직접 작성할 수 없다'
);
...
SELECT throws_ok(
  $$INSERT INTO public.chat_messages
      (user_id, conversation_id, role, content)
    VALUES
      ('00000000-0000-4000-8000-000000000005', gen_random_uuid(), 'assistant', '위조된 AI 답변')$$,
  '42501',
  'new row violates row-level security policy for table "chat_messages"',
  '본인이 assistant 역할의 과거 AI 답변을 위조할 수 없다'
);
