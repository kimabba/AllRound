-- 대회 검색은 입력어와 검색 대상의 공백 유무가 달라도 같은 결과를 반환한다.
--
-- 예: 저장된 제목이 `광주 오픈`일 때 `광주오픈`으로 검색해도 찾을 수 있어야 한다.
-- 기존 LIKE 메타문자(%, _) 이스케이프는 그대로 유지해 와일드카드로 해석되지
-- 않게 한다. 함수 시그니처와 나머지 필터/정렬 규칙은 변경하지 않는다.
...
AS $function$
  ...
  -- 원문 검색어와 모든 공백을 제거한 검색어를 한 번만 계산한다.
  -- 두 값 모두 LIKE 메타문자를 이스케이프해 리터럴 검색 규칙을 보존한다.
  CROSS JOIN LATERAL (
    SELECT
  ...
  WHERE t.status = 'published'
    ...
    AND (
      ...
      OR regexp_replace(t.title, '[[:space:]]+', '', 'g')
        ILIKE '%' || q.compact_term || '%' ESCAPE '\'
      OR regexp_replace(COALESCE(t.organizer, ''), '[[:space:]]+', '', 'g')
        ILIKE '%' || q.compact_term || '%' ESCAPE '\'
      OR regexp_replace(COALESCE(t.description, ''), '[[:space:]]+', '', 'g')
        ILIKE '%' || q.compact_term || '%' ESCAPE '\'
      OR regexp_replace(COALESCE(t.region, ''), '[[:space:]]+', '', 'g')
        ILIKE '%' || q.compact_term || '%' ESCAPE '\'
      OR regexp_replace(COALESCE(t.location, ''), '[[:space:]]+', '', 'g')
        ILIKE '%' || q.compact_term || '%' ESCAPE '\'
    )
    AND (
      NOT p_only_my_grade
