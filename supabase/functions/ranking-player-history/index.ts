...
import { fetchPlayerHistory } from '../_shared/crawler/parsers/gnuboard_player_history.ts';
...
Deno.serve(withCors(async (req) => {
  ...
  try {
    ...
    const { error: replaceError } = await svc.rpc('replace_org_player_history', {
      p_org: params.orgCode,
      p_org_player_id: params.orgPlayerId,
      p_is_complete: !fetched.reachedPageLimit,
      p_rows: fetched.rows.map((row) => ({
    ...
    if (replaceError) {
      console.error('[ranking-player-history] cache replace failed:', replaceError.message);
      return errorResponse('선수 이력을 저장하지 못했습니다', 500);
    }
    return jsonResponse({
      results: fetched.rows.map((row) => ({
