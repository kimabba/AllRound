...
export type OrgPlayerLinkStatus = 'pending' | 'confirmed' | 'rejected';
...
/** `org_player_links`에서 본인 랭킹 판별에 필요한 필드만 고른 행. */
export interface OrgPlayerLinkRow {
  org_code: RankingOrgCode;
  org_player_id: string;
  user_id: string;
  status: OrgPlayerLinkStatus;
}
...
const LINK_STATUSES = new Set<OrgPlayerLinkStatus>(['pending', 'confirmed', 'rejected']);
...
function parseOrgPlayerLinkRow(value: unknown, index: number): OrgPlayerLinkRow {
  if (
    !isRecord(value) ||
    !isRankingOrgCode(value.org_code) ||
    typeof value.org_player_id !== 'string' ||
    value.org_player_id.length === 0 ||
    typeof value.user_id !== 'string' ||
    value.user_id.length === 0 ||
    typeof value.status !== 'string' ||
    !LINK_STATUSES.has(value.status as OrgPlayerLinkStatus)
  ) {
    throw new TypeError(`org_player_links[${index}] 형식이 올바르지 않습니다`);
  }
  return {
    org_code: value.org_code,
    org_player_id: value.org_player_id,
    user_id: value.user_id,
    status: value.status as OrgPlayerLinkStatus,
  };
}
...
/** Supabase의 외부 JSON 경계에서 `org_player_links` 행을 즉시 좁힌다. */
export function parseOrgPlayerLinks(value: unknown): OrgPlayerLinkRow[] {
  if (!Array.isArray(value)) throw new TypeError('org_player_links 결과는 배열이어야 합니다');
  return value.map(parseOrgPlayerLinkRow);
}
...
/**
 * 본인 연결 상태와 현재 랭킹을 함께 해석한다.
 * confirmed 연결만 내 랭킹으로 인정하며 pending/rejected를 순위처럼 노출하지 않는다.
 */
export function renderMyRankingResults(
  rows: readonly OrgRankingRow[],
  links: readonly OrgPlayerLinkRow[],
  currentUserId: string,
): string {
  const ownLinks = links.filter((link) => link.user_id === currentUserId);
  const confirmed = ownLinks.filter((link) => link.status === 'confirmed');
  if (confirmed.length === 0) {
    if (ownLinks.some((link) => link.status === 'pending')) {
      return '본인 랭킹 연결 승인을 기다리고 있습니다. 승인되면 현재 순위와 포인트를 확인할 수 있습니다.';
    }
    return '아직 본인으로 확정된 협회 랭킹이 없습니다. 랭킹 화면에서 본인 연결을 신청해 주세요.';
  }

  const confirmedKeys = new Set(
    confirmed.map((link) => `${link.org_code}:${link.org_player_id}`),
  );
  const ownRows = rows.filter((row) =>
    row.org_player_id !== null &&
    confirmedKeys.has(`${row.org_code}:${row.org_player_id}`)
  );
  if (ownRows.length === 0) {
    return '본인 연결은 확인됐지만 현재 협회 랭킹표에서 순위 정보를 찾지 못했습니다.';
  }
  return ['내 현재 협회 랭킹입니다.', ...renderRows(ownRows)].join('\n');
}
