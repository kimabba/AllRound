...
Deno.test('routing 가능 여부: tournament_search/detail + club_search 가 LLM 우회 대상', () => {
  // chat/index.ts 의 ROUTABLE_INTENTS 미러:
  //   intent ∈ {tournament_search, tournament_detail, club_search} && confidence ≥ 0.95.
  // 룰 분류는 confidence 1.0 → 해당 의도면 routable.
  const ROUTABLE = new Set<Intent>(['tournament_search', 'tournament_detail', 'club_search']);
  const wouldRoute = (r: IntentResult): boolean => ROUTABLE.has(r.intent) && r.confidence >= 0.95;
  ...
  // 대회 검색/상세 + 클럽 검색 룰 매칭이 LLM 없이 routing 된다.
  // (match_schedule 은 별도 결정적 안내 경로 — routing 셋에는 없음)
