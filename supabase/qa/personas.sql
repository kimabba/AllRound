...
-- scripts/qa/assert_local_supabase.sh를 통과한 뒤 로컬 DB 컨테이너에만 실행한다.
...
BEGIN
  ...
  SELECT
    ...
    '', '', '', '', ''
  FROM (VALUES
    ('00000000-0000-4000-8000-000000000001'::uuid, 'qa-admin@allround.invalid',     'QA 관리자',     '1990-01-01'),
    ('00000000-0000-4000-8000-000000000002'::uuid, 'qa-owner@allround.invalid',     'QA 오너',       '1990-01-01'),
    ('00000000-0000-4000-8000-000000000003'::uuid, 'qa-manager@allround.invalid',   'QA 매니저',     '1990-01-01'),
    ('00000000-0000-4000-8000-000000000004'::uuid, 'qa-delegate@allround.invalid',  'QA 위임회원',   '1990-01-01'),
    ('00000000-0000-4000-8000-000000000005'::uuid, 'qa-member@allround.invalid',    'QA 일반회원',   '1990-01-01'),
    ('00000000-0000-4000-8000-000000000006'::uuid, 'qa-applicant@allround.invalid', 'QA 가입신청자', '1990-01-01'),
    ('00000000-0000-4000-8000-000000000007'::uuid, 'qa-offender@allround.invalid',  'QA 제재대상',   '1990-01-01'),
    ('00000000-0000-4000-8000-000000000008'::uuid, 'qa-empty@allround.invalid',     'QA 미완성계정', NULL)
  ) AS persona(id, email, display_name, birth_date)
...
FROM (VALUES
  ...
  ('00000000-0000-4000-8000-000000000007'::uuid, 'QA 제재대상',  'QA 제재대상',  'busan')
...
VALUES
  ('00000000-0000-4000-8000-000000000101', 'tennis', 'QA 광주 일반부 공개대회',
   current_date + 14, current_date + 7, '광주', 'gwangju', ARRAY['gj_m_general', 'kata_4'],
