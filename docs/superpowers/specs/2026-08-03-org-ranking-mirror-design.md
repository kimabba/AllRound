...
- [ ] 탈퇴 시 계정 연결 즉시 소멸 (`on delete cascade`). 단 **미러의 해당 행은 남는다** —
      ...
      삭제(이의) 요청 채널을 타야 한다
> **철회됨 (2026-08-03)**: 초안에는 "협회 동의를 문서로 확보"가 네 번째 항목으로 있었다.
> 그건 Commander 요청이 아니라 **드론이 추가한 것**이고, Commander 지시로 항목과 문서
> (`docs/legal/org-ranking-crawl-consent.md`)를 모두 뺐다. 협회 동의 자체는 Commander가
> 이미 확보했다(결정 6) — 별도 문서화는 요구되지 않았다.
