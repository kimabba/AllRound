...
작성 2026-07-11 · **상태: 설계 확정(unmapped 결정 A 확정). writing-plans 진행.**
...
**부서가 사전과 하나도 안 맞을 때: `eligible_grades=[]`(비움) + draft 검수.**
- 원문은 `division_label_local`에 저장. 어드민이 게시 전 draft에서 부서 보정.
- "추측 안 함" 원칙 + draft 흐름과 정합. 잘못 게시돼도 둘러보기엔 뜨고 등급매칭만 안 됨(저위험).
- 대안 B(전등급 노출)는 draft 검수가 이미 안전망이라 이점 없고 노이즈만 증가 → 기각.
- **`mapDivisionsByDict`는 unmapped 시 `codes=[]` 반환.** 파서는 그 값을 그대로 `eligible_grades`에 넣음(빈 배열 허용). 기존 "오픈+일반 기본값" 폴백은 제거.
