...
**기존 published 50건 검수 스테이징**: 이 행들은 format worker가 결과를 `format_staged`에 넣고 `needs_review`로 둔다(콘텐츠 미변경 → 사용자엔 기존 표시 유지). 어드민 검수 UI에서 원문 대비 승인 시 staged→실제 콘텐츠 반영 + `formatted`. **신규 크롤 유입·draft 승인분은 자동+검증 즉시 반영**(백필 대상 아님).
...
| # | 결정 | 채택 |
|---|---|---|
| 1 | 기존 published 50건 반영 | 검수 스테이징(staged→승인→반영) |
