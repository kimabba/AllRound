...
17. 대회·클럽·볼보이 3개 하단 메뉴와 상단 설정 아이콘을 통한 MY 진입
...
35. 하단 3개 메뉴·문맥 토글·어두운 전체 채팅·출처 링크의 공식 Flutter
    레이블·대비·iOS 44px·Android 48px 접근성 기준
36. 협회 랭킹 응답 순서와 무관하게 1위가 맨 위인지, 선수 행을 열면 연도별 최신순
    대회 이력이 보이는지, 200% 글자 크기에서도 잘림·오버플로가 없는지 검증
37. 명백한 성적·욕설·위험 채팅은 저장·임베딩·Gemini 전송 전에 차단되고, `대회 성적`·
    `성인부`·`서브 폭발력` 같은 정상 스포츠 표현은 허용되는지 검증

웹과 macOS 모두 화면 문구가 아닌 고정 E2E key를 사용하며, 로그인·가입·온보딩·대회·클럽·볼보이 하단 메뉴·상단 설정을 통한 MY 진입·보조 라우트를 실제 pointer tap으로 검증한다. 비동기 보조 화면은 로딩이 끝난 ready key를 기다린다. 실제 데이터 소스가 없는 친구 일정 프리뷰는 출시 라우트와 시각 증거에서 제외하고, MY 화면·계정 설정을 대신 검증한다. OAuth·카메라·알림 권한처럼 운영체제 팝업이 필요한 흐름은 Patrol 계층에서 추가한다.
