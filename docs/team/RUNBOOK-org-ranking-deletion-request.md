...
### ✅ 출시본 개인정보 연락처 정정 확인 (2026-08-04)

현재 확인된 iOS 출시본(1.0.0 (5))과 App Store 제품 페이지는 모두 최신 개인정보
처리방침 주소와 연락처(`play@jyoungad.kr`)를 사용한다. 옛 주소
(`demian.772@gmail.com`)는 현재 앱과 스토어에 더 이상 표시되지 않는다.

다만 과거 출시본이나 이전 안내를 본 이용자가 옛 주소로 요청할 수 있으므로, 기존
요청을 놓치지 않도록 옛 주소의 수신·자동전달 유지 여부는 운영자가 별도로 확인한다.
