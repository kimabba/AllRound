...
| 승인 완료·출시 대기 버전 | `1.0.1 (6)` | 백과장 업로드·심사 제출, 2026-09-03 기준 심사 승인 완료·"개발자 출시 대기 중". 사용자에게는 아직 미출시 |
| 준비 중 버전 | `1.1.0 (7)` | 코드 반영 완료(PR #502), 1.0.1 정리 후 새 버전 생성·업로드 예정 |
...
- 빌드 5에는 최소 지원 빌드 안내가 없다. 서버의 최소 빌드 설정으로 이 버전을 강제
  업데이트할 수 없다.
- `1.0.1 (6)`이 "개발자 출시 대기 중" 상태로 남아 있는 동안에는 App Store Connect에서
  새 버전(`1.1.0`)을 만들 수 없다. 1.0.1을 그대로 출시할지, 출시하지 않고 정리할지는
  kimabba와 백과장이 함께 정한다.
...
| `1.0.1` | `6` | 심사 승인·개발자 출시 대기 | — | 백과장 업로드(경위 확인 중) | pubspec은 `1.1.0+6`이었으나 IPA 사용자 버전은 `1.0.1`. 처리방침 URL은 옛 GitHub Pages 주소 |
| `1.1.0` | `7` | 코드 준비 완료 | — | PR #502 | 약관·처리방침 주소를 all-round.it.kr 로 교체. 1.0.1 정리 후 새 버전 생성·업로드 |
