...
| 현재 배포 버전 | `1.0.1 (6)` | 2026-09-03 출시 완료. 직전 `1.0.0 (5)`는 2026-08-04 출시 |
| 심사 진행 버전 | `1.1.0 (7)` | 2026-09-03 업로드·메타데이터 완료, 심사 제출 진행. 처리방침·지원 URL을 `all-round.it.kr`로 반영 |
...
- 빌드 5에는 최소 지원 빌드 안내가 없다. 서버의 최소 빌드 설정으로 이 버전을 강제
  업데이트할 수 없다.
- `1.0.1 (6)`은 2026-09-03 출시했다. 출시 대기 버전이 있으면 App Store Connect에서
  새 버전을 만들 수 없어, 1.0.1을 먼저 출시한 뒤 `1.1.0 (7)` 버전을 생성·업로드했다.
...
다음 버전은 `1.1.0+6`으로 정했다(2026-08-18). 빌드 5 이후 대회 탭 개편·협회 랭킹
본인 연결·챗봇 기능 추가처럼 기능 묶음이 쌓여 수정판(`1.0.1`)이 아니라 가운데 숫자를
올렸다. `appBuildNumber`도 같은 `6`으로 맞춰야 하며, 회귀 테스트가 두 값을 검사한다.
2026-09-03 정정: 빌드 6은 App Store Connect에 사용자 버전 `1.0.1`로 업로드·심사 승인된
상태였다(개발자 출시 대기). pubspec은 `1.1.0+6`이었지만 IPA의 사용자 버전은 `1.0.1`로
올라갔다(빌드 시 `--build-name` 지정으로 추정, 경위는 백과장 확인 필요). 빌드번호 6은
이미 소진됐으므로 다음 빌드는 `1.1.0+7`이다(`appBuildNumber`도 `7`).
...
0. 새 버전을 만들기 전에 "개발자 출시 대기 중" 같은 미출시 이전 버전이 없는지 확인한다.
   남아 있으면 새 버전을 만들 수 없다(§1 주의 사항 참고).
...
5. 변경사항, 스크린샷, 심사 노트, 수출 규정 질문을 확인한다.
   - 개인정보 처리방침 URL은 버전별 항목이라 새 버전을 만들 때마다 입력·확인한다.
     승인된 버전은 수정이 잠긴다. `1.1.0`부터 `https://all-round.it.kr/privacy/`를 입력한다.
...
| `1.0.1` | `6` | 출시 완료 | 2026-09-03 | 백과장 출시 | pubspec은 `1.1.0+6`이었으나 IPA 사용자 버전은 `1.0.1` |
| `1.1.0` | `7` | 업로드·심사 제출 진행 | — | PR #502 | 2026-09-03 백과장 Mac에서 `make release-ios`(.env.production)로 빌드→Xcode 업로드. 스크린샷 iPhone 7·iPad 4, 처리방침·지원 URL `all-round.it.kr` 입력 |

새 빌드를 출시하면 위 표에 반드시 한 줄을 추가한다. 심사 반려·TestFlight
검증용 빌드도 빌드번호와 상태를 기록해 다음 번호를 누구나 알 수 있게 한다.

2026-09-03 실측 정정: 이 문서는 빌드 6을 "미업로드"로 적고 있었으나 App Store Connect
확인 결과 `1.0.1 (6)`으로 이미 승인·출시 대기 상태였다. 문서 맨 위 원칙(실제 상태가
다르면 문서를 먼저 갱신)을 적용한 사례다.

2026-09-03 후속: 위 확인 뒤 `1.0.1 (6)`을 출시하고, `1.1.0 (7)`을 백과장 Mac에서 빌드·
업로드했다. 서명 인증서(`Apple Distribution: J Young Consulting`)와 App Store 프로파일이
백과장 Mac에 이미 있어, 드론 Mac으로 인증서를 옮기지 않고 여기서 바로 서명·업로드했다.
릴리스 빌드는 반드시 `.env.production`(`make release-ios`)을 쓴다 — `.env.local`은 개발용
로컬 DB라 스토어 빌드에 넣으면 안 된다.
