...
- [ ] **[`dependencies.md`](dependencies.md) 대장에서 `iOS 실기기 확인 = 미확인`인 네이티브 부품이 없는지 확인.** 네이티브 부품은 안드로이드에서 잘 돌아도 iOS 에서 다르게 동작한다(권한 기본값·크래시). 2026-08-05 기준 `webview_flutter`·`cupertino_http`·`firebase_crashlytics` **셋**이 미확인 상태로 들어와 있다.
