...
- 의도 10종: `tournament_search`, `tournament_detail`, `ranking_lookup`, `my_ranking`,
...
- `my_ranking`은 현재 사용자의 `org_player_links.status='confirmed'` 연결만 본인으로
  인정한다. `pending`과 미연결 상태를 실제 순위처럼 추측하지 않는다.
...
| `chat_route` | 대회 라우팅 이벤트, `ranking_lookup_routed`, `my_ranking_routed`, 랭킹 조회 오류 |
