...
- `clubs_screen.dart` — "내 클럽" / "클럽 찾기" 탭 + FAB(클럽 만들기)
  ...
  - 추천순·회원 많은 순·최근 등록순·이름순 정렬을 제공하고 마지막 선택을 기기에 저장
...
- 활성 회원과 운영진은 자유·모집·사진 게시글을 작성할 수 있음
- 공지사항은 owner/manager 또는 공지 권한이 있는 회원만 작성 가능
- 일반 게시글에는 모든 활성 회원이 댓글을 작성할 수 있음
- 공지사항은 댓글 대상에서 제외하며, 모임 일정은 `club_events` 별도 탭에서 관리
- 탈퇴한 작성자의 게시글·댓글은 내용을 보존하고 작성자를 `탈퇴한 사용자`로 표시

## 멤버 채팅

- 활성 멤버는 클럽 단체 채팅과 같은 클럽 멤버와의 1:1 채팅을 사용할 수 있다.
- `club_chat_messages`는 Supabase Realtime 발행 대상이며, 앱은 새 메시지를 실시간으로 반영한다.
- RLS가 현재 활성 멤버 여부와 1:1 참여자 여부를 확인하고 차단한 사용자끼리는 대화할 수 없다.
- 메시지는 1,000자 이하이며 사용자별 분당 20개로 제한된다. 30초 조회는 실시간 연결 장애에 대비한 보조 수단이다.

## UGC 신고·차단·제재

- 사용자는 클럽 게시글, 댓글, 모임을 신고하고 상황 캡처를 최대 3장 첨부할 수 있다.
- 신고 접수 시 서버가 원문과 주변 댓글을 `content_snapshot`에 보존한다. 증거 이미지는 `ugc-report-evidence` private bucket을 사용한다.
- 사용자 차단은 양방향 노출을 제한하며, 더보기 > 차단 관리에서 해제한다.
- 관리자는 `/admin/reports`에서 신고 원문·첨부 이미지를 확인하고 콘텐츠 삭제, 댓글 제한, 클럽 가입 제한, 커뮤니티 작성 제한을 7일·30일·영구로 적용하거나 해제한다.
- 커뮤니티 작성·클럽 가입 전 `2026-07-15` 버전 약관의 명시적 필수 동의가 필요하며 RLS와 Edge Function에서 재검증한다.
