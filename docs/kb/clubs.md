...
클럽 개설자는 `pending` 또는 `approved` 상태에서 항상 active owner 멤버십을 유지한다. 과거 데이터의 누락·left owner 행은 `20260716161000_repair_club_owner_memberships.sql`로 복구하며, 승인 함수도 승인 전에 owner 멤버십을 upsert한다.
