...
- 랭킹 목록은 협회·부서 안에서 `rank` 오름차순이 정본이며 1위부터 표시한다.
- 현재 랭킹은 `org_rankings`, 선수별 과거 대회 성적은 `org_player_results`로 분리한다.
- 선수별 이력은 랭킹 행을 열 때만 협회 공개 페이지에서 가져오며 24시간 동안 재사용한다.
- 이력 화면은 현재 순위·부서·누적 포인트를 먼저 요약하고, 대회 결과는 최신 연도부터
  날짜·대회·종목/부서·성적·획득 포인트 순으로 보여준다.
